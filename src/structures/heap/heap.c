...
void imprimirHeapFirstFive( Heap heap)
{
	printf("\n HEAP: \n");
	if(heap.cantidad)
	{
...
Arbol heapToTree(Heap *h,void (*imprimir)(void*),int (*comparar)(void*,void*)){
	if(h->cantidad < 1)
		return (Arbol){0};
	
	NodoA *nodos[h->cantidad];
	//esto por que el make se enoja
	nodos[0] = NULL;

	for(int i =0;i < h->cantidad;i++){
		nodos[i] = crearNodoA(h->datos[i]);
		//Inicialziacion de nulos a sus hijos
		nodos[i]->izq = nodos[i]->dch = NULL; 
	}

	for(int i =0;i <=( h->cantidad/2 - 1);i++){
		nodos[i]->izq = nodos[i*2 + 1];
		if((i*2 + 2) >= h->cantidad)
			break;	
		nodos[i]->dch = nodos[i*2 + 2];
	}
	return (Arbol){nodos[0],0,imprimir,comparar,NULL};
}